...
import { Food } from "file:///c:/Users/nitin/OneDrive/Desktop/projects/Work/FNC/fnc-backend-main/fnc-backend-main/src/models/food/food.model.js";
import { Nutrient } from "file:///c:/Users/nitin/OneDrive/Desktop/projects/Work/FNC/fnc-backend-main/fnc-backend-main/src/models/nutrient.model.js";

dotenv.config({ path: 'c:/Users/nitin/OneDrive/Desktop/projects/Work/FNC/fnc-backend-main/fnc-backend-main/.env' });
...
async function updateDatabase() {
    try {
        ...
        // Get Nutrients
        const nutrients = await Nutrient.find({ type: 'macro' });
        const nutrientMap = {};
        nutrients.forEach(n => nutrientMap[n.name] = n._id);

        if (!nutrientMap.protein || !nutrientMap.carbohydrate || !nutrientMap.fat || !nutrientMap.fiber) {
            console.error("Critical nutrients missing in database. Run seed script first.");
            process.exit(1);
        }
        ...
        for (const [foodName, data] of Object.entries(FOOD_NUTRITION_MAP)) {
            const food = await Food.findOne({ name: foodName });
            if (food) {
                console.log(`Updating ${foodName}...`);
                
                food.calories_per_quantity = data.cal;
                food.nutrients_per_quantity = 100; // Reset to standard 100g base if needed
                food.nutrients = [
                    { nutrient_id: nutrientMap['protein'], quantity: data.p },
                    { nutrient_id: nutrientMap['carbohydrate'], quantity: data.c },
                    { nutrient_id: nutrientMap['fat'], quantity: data.f },
                    { nutrient_id: nutrientMap['fiber'], quantity: data.fib }
                ];
        ...
        console.log(`Updated: ${updatedCount} foods`);
        console.log(`Skipped (not in DB): ${skipCount} foods`);
