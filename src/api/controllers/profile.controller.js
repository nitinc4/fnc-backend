...
async function getDietPlans(healthIssues, user) {
    ...
    // Assign specific diet plans based on health issues ONLY if the user is on a paid plan
    if (isPaidPlan && healthIssues && healthIssues.length > 0) {
        const issueIds = healthIssues.map(issue => issue._id ? issue._id.toString() : issue.toString());
    ...
    // If free plan, OR if paid but no specific diet plan exists for the selected health issues, assign the general plan
    ...
    static async createProfile(req, res) {
        ...
        try {
            ...
            for (let issue of health_issues) {
                ...
                // FIXED: Pushing only the Object ID so Mongoose saves it correctly
                healthIssues.push(healthIssue._id) 
            }
            ...
            const createdUser = await UserProfile.create({
                ...
                diet_plans: dietPlans.map(plan => plan._id) // Store IDs for plans too
            })
    ...
    static async updateProfile(req, res) {
        ...
        try {
            ...
            if (diet_plans) {
                ...
                let dietPlanList = []
                // FIXED: Change logic flaw `diet_plans.length === 0` to `diet_plans.length > 0`
                if (diet_plans.length > 0) {
                    ...
                    existingUserProfile.diet_plans = resolvedPlans.map(plan => plan._id)
                }
            } else {
                // FIXED: Always recalculate auto-assignment if `diet_plans` is not explicitly requested.
                // This guarantees the diet plan changes seamlessly if a user modifies ONLY their health issues.
                const resolvedPlans = await getDietPlans(existingUserProfile.health_issues, user)
                existingUserProfile.diet_plans = resolvedPlans.map(plan => plan._id)
            }
